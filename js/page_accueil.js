...
function getLogin(callback) {
    ...
    xhr.open("POST", "http://35.183.168.181:80/login");
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.setRequestHeader("cache-control", "no-cache");
    xhr.setRequestHeader("Postman-Token", "ccf2154c-3251-4b66-828f-ba4fd2ea285f");
...
function verifierLogin(login) {
    return login.httpStatus === 200;
}

function ouvertureSession(user, pass) {
    let passMD5 = MD5.MD5(pass);
    utilisateur.login = user;
    utilisateur.password = passMD5
    alert(getLogin(verifierLogin));
...
function loadPage() {
    ...
    btnLogin.addEventListener("click", function () {
        let user = document.getElementById("inputNomUtilisateur");
        let pass = document.getElementById("inputMotDePass");
        ouvertureSession(user.value, pass.value);
    });
